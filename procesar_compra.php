...
<body>
    <div class="container">
        ...
        if ($producto) {
            ...
            // Registrar la compra
            $sql_compra = "INSERT INTO compras (producto_id, cantidad, total) VALUES ($producto_id, $cantidad, $total)";
            if ($conn->query($sql_compra) === TRUE) {
                // Actualizar el stock del producto (aumentarlo)
                $nuevo_stock = $producto['stock'] + $cantidad;
                $sql_actualizar = "UPDATE productos SET stock = $nuevo_stock WHERE id = $producto_id";
                $conn->query($sql_actualizar);

                echo "<div class='message'>Compra registrada con éxito. Total: $" . $total . "</div>";
            } else {
                echo "<div class='message'>Error al registrar la compra: " . $conn->error . "</div>";
            }
        } else {
